...
public class VerseTests : BaseIntegrationTest
{
    ...
    public async Task VerseTest_InsertGetUpdate()
    {
        ...
        var verse = await verseContext.GetVerse(newVerse.Reference);
        ...
        await verseContext.UpdateUsersMemorizedVerse(verse.Reference);
        verse = await verseContext.GetVerse(newVerse.Reference);
        Assert.NotNull(verse);
        Assert.Equal(1, verse.UsersMemorizedCount);

        await verseContext.UpdateUsersSavedVerse(verse.Reference);
        verse = await verseContext.GetVerse(newVerse.Reference);
        Assert.NotNull(verse);
