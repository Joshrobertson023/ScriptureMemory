...
{
    //Task<IEnumerable<Verse>> GetAllVerses(int offset, int nextFetch);
    //Task<Verse?> GetVerse(string reference);
    //Task<IEnumerable<Verse>> GetChapterVerses(string book, int chapter);
    //Task InsertVerse(Verse verse);
    //Task UpdateUsersMemorizedVerse(string reference);
    //Task UpdateUsersSavedVerse(string reference);
    //Task ExportVersesToJsonFile(string filePath);
    //Task<SearchData> GetVerseSearchResults(string search);
    //Task<Verse?> GetVerseFromId(int id);
    //Task UpdateVerseText(string text, int rownum);
    //Task<List<Verse>> GetAllVersesFromReferenceList(List<string> references);
    //Task<IEnumerable<Verse>> GetTopSavedVerses(int top);
    //Task<IEnumerable<Verse>> GetTopMemorizedVerses(int top);
