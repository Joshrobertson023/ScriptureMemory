using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Text.Json;
using System.Threading.Tasks;
using ScriptureMemory.Server.Tools;

namespace DataAccess.Models;

/// <summary>
/// To create a log, pass in: 
/// Username [required], ActionType [required], EntityType [required],
/// EntityId, ContextDescription, JsonMetadata, 
/// SeverityLevel (optional), IsAdminAction (optional)
/// </summary>
public sealed class ActivityLog
{
    public int Id { get; set; } = 0;
    public int? UserId { get; set; }
    public Enums.ActivityLogType LogType { get; set; }
    public Enums.EntityType? EntityType { get; set; }
    public int? EntityId { get; set; }
    public string? JsonMetadata { get; set; }
    public DateTime Timestamp { get; set; } = DateTime.UtcNow;

    public ActivityLog() { }

    public ActivityLog(Enums.ActivityLogType actionType, Enums.EntityType entityType)
    {
        LogType = actionType;
        EntityType = entityType;
    }

    public ActivityLog(int userId, Enums.ActivityLogType actionType, Enums.EntityType entityType)
    {
        UserId = userId;
        LogType = actionType;
        EntityType = entityType;
    }

    public ActivityLog(Enums.ActivityLogType actionType, Enums.EntityType entityType, int entityId)
    {
        LogType = actionType;
        EntityType = entityType;
        EntityId = entityId;
    }

    public ActivityLog(int userId, Enums.ActivityLogType actionType, Enums.EntityType entityType, int entityId)
    {
        UserId = userId;
        LogType = actionType;
        EntityType = entityType;
        EntityId = entityId;
    }

    public ActivityLog(
        int? userId,
        Enums.ActivityLogType actionType,
        Enums.EntityType entityType,
        int? entityId,
        string? contextDescription,
        object? jsonMetadata,
        Enums.SeverityLevel severityLevel = Enums.SeverityLevel.Info,
        bool isAdminAction = false)
    {
        UserId = userId;
        LogType = actionType;
        EntityType = entityType;
        EntityId = entityId;
        if (jsonMetadata is not null)
            JsonMetadata = JsonSerializer.Serialize(jsonMetadata);
    }

    public override string ToString()
    {
        StringBuilder returnString = new();

        if (Id != 0)
            returnString.Append("Id: ").Append(Id).Append("\n");
        if (UserId is not null)
            returnString.Append("UserId: ").Append(UserId).Append("\n");
        returnString.Append("ActionType: ").Append(LogType).Append("\n");
        returnString.Append("EntityType: ").Append(EntityType).Append("\n");
        if (EntityId is not null)
            returnString.Append("EntityId: ").Append(EntityId).Append("\n");
        if (!string.IsNullOrWhiteSpace(JsonMetadata))
            returnString.Append("JsonMetadata: ").Append(JsonMetadata).Append("\n");
        returnString.Append("CreatedAt: ").Append(Timestamp).Append("\n");

        return returnString.ToString();
    }
}
