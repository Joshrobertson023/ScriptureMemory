...
// import { ScrollView, View, Text, Pressable, Modal, TouchableOpacity, ActivityIndicator } from "react-native";
...
// const PracticeSessionScreen = ({route}: Props) => {
//     const {practicingPassage} = route.params;
...
//     const {getVerseTypingPart} = useReferenceParser();
...
//     const [stageAccuracy, setStageAccuracy] = useState<{
//         accuracy: number;
//         correct: number;
//         total: number;
//     } | null>(null);
...
//           if (!practicingPassage?.passage.verses?.length) {
//             // TODO: Alert with copy message
//             return;
//           }
    
//           setAccuracyModalVisible(false);
          
//           setBook(practicingPassage.passage.reference.book);
//           setChapter(practicingPassage.passage.reference.chapter.toString());
//           setVersesTypeParts(getVerseTypingPart(practicingPassage.passage.reference.readableReference));

//           let text = '';
//           const verseTexts: string[] = practicingPassage.passage.verses.map((v) => v.text);
//           for (const vt in verseTexts) {
//             text += vt;
//           }
//           setPassageText(text);

//           setReadableReference(
//             practicingPassage.passage.reference.readableReference.substring(
//               practicingPassage.passage.reference.readableReference.indexOf(":") + 1,
//             ) || "",
//           );
...
//           const percentStage1 = 0.0;
//           const percentIncrement = 0.3;
...
//           const stage2Count = Math.floor(
//             totalLength * (percentStage1 + percentIncrement),
//           );
...
//           const stage3Count = Math.floor(
//             totalLength * (percentStage1 + percentIncrement * 2.5),
//           );
...
//       }, [practicingPassage, learnVerseReference, totalStages]);
...
//           const fullRef = practicingPassage?.readableReference ?? "";
...
//             activeBookIndex < bookWords.length &&
//             !learnedBook
//           ) {
...
//           if (
//             user.typeOutReference &&
//             activeReferenceIndex < referenceWords.length &&
//             !learnedVerseReference &&
//             learnedBook
//           ) {
...
//           if (activeIndex >= allWords.length) return;
...
//             const currentAccuracy = computeStageAccuracy(
//               updatedWords,
//               referenceWords,
//               bookWords,
//             );
...
//               const correctWords = allWordsToCheck.filter(
//                 (w) => w.isCorrect === true,
//               ).length;
//               const totalWords = allWordsToCheck.length;
//               const localAccuracy =
//                 totalWords > 0 ? Math.floor((correctWords / totalWords) * 100) : 0;
...
//                 finalStageAccuracies.reduce((a, b) => a + b, 0) /
//                   finalStageAccuracies.length,
//               );
//               setAccuracy(sessionAccuracy);
      
//               // Check if this is a verse of day (no id or no collectionId)
//               const isVerseOfDay =
//                 !practicingPassage?.id || !practicingPassage?.collectionId;
      
              
//                 // Handle regular UserVerse memorization
//                 const info: MemorizedInfo = {
//                   userVerseId: practicingPassage?.id || 0,
//                   accuracy: sessionAccuracy,
//                 };
      
//                 const userVerseMemorizedInfo: UserVerseMemorizedInfo =
//                   await memorizePassage(info);
//                 setPointsGained(userVerseMemorizedInfo.pointsGained);
//                 setNextDue(
//                   userVerseMemorizedInfo.userVerse.dueDate
//                     ? new Date(userVerseMemorizedInfo.userVerse.dueDate)
//                     : new Date(),
//                 );
//                 setTimesMemorized(
//                   userVerseMemorizedInfo.userVerse.timesMemorized || 0,
//                 );
...
//                   .getState()
//                   .collections.find(
//                     (c) => c.collectionId === practicingPassage?.collectionId,
//                   );
//                 if (collection) {
//                   const colToSend = {
//                     ...collection,
...
//               const errorMessage =
//                 error instanceof Error ? error.message : "Failed to memorize passage";
...
//         <Pressable
//             onPress={() => {
//             handleKeyboardPress(char);
//             }}
//             style={{
//             width: "8%",
//             height: 50,
//             backgroundColor: theme.colors.verseText,
//             borderRadius: 8,
...
//                 w.isCorrect === null ? (
//                   w.isHint ? (
//                     <Text key={w.id} style={{ color: theme.colors.verseHint }}>
//                       {w.word}
//                     </Text>
//                   ) : (
//                     <Text key={w.id} style={{ color: theme.colors.background }}>
//                       {w.word}
//                     </Text>
//                   )
//                 ) : w.isCorrect ? (
//                   <Text key={w.id} style={{ color: theme.colors.onBackground }}>
//                     {w.word}
//                   </Text>
//                 ) : (
//                   <Text key={w.id} style={{ color: theme.colors.error }}>
//                     {w.word}
//                   </Text>
//                 ),
//               )}
...
//           >
//             <Text
//               style={{ ...styles.subheading, marginBottom: 10, marginTop: 20 }}
//             >
//               Accuracy: {accuracy}%
//             </Text>
//             {accuracy >= 90 ? (
...
//                   Good job! Go ahead and finish the session.
//                 </Text>
//               ) : (
//                 <Text style={{ ...styles.text, textAlign: "center" }}>
//                   Good job! Go ahead and continue to the next stage.
//                 </Text>
//               )
//             ) : accuracy >= 75 ? (
//               <Text style={{ ...styles.text, textAlign: "center" }}>
//                 Nice job, but you can get a better score. Try again!
//               </Text>
//             ) : (
//               <Text style={{ ...styles.text, textAlign: "center" }}>
//                 It's recommended that you retry this stage.
//               </Text>
//             )}
