...
export default function PracticeSessionScreen() {
  ...
  const setShouldReloadPracticeList = useAppStore(
    (state) => state.setShouldReloadPracticeList,
  );
  ...
  const versesMemorized = useAppStore(
    (state) => state.user?.versesMemorized || 0,
  );
  const [animatedPoints, setAnimatedPoints] = useState(points);
  const [animatedVersesMemorized, setAnimatedVersesMemorized] =
    useState(versesMemorized);
  ...
  useEffect(() => {
    const getUserVersePracticing = async () => {
      ...
      const userVerseParts: UserVerseParts =
        await getUserVerseParts(selectedUserVerse);
      ...
      setReadableReference(
        selectedUserVerse.readableReference.substring(
          selectedUserVerse.readableReference.indexOf(":") + 1,
        ) || "",
      );
  ...
  const nextStage = async () => {
    ...
    if (currentStage >= totalStages) {
      try {
        ...
        const isVerseOfDay =
          !selectedUserVerse?.id || !selectedUserVerse?.collectionId;
        ...
        if (
          ...
          const collection = useAppStore
            .getState()
            .collections.find(
              (c) => c.collectionId === selectedUserVerse?.collectionId,
            );
          if (collection) {
            const colToSend = {
              ...collection,
          ...
          >
            ...
            >
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Text style={styles.buttonText_filled}>Back to Home</Text>
              </TouchableOpacity>
            </TouchableOpacity>
          </TouchableOpacity>
        </Modal>
